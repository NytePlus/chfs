...
public:
  ...
  virtual auto write_block_to_memory(block_id_t block_id, const u8 *block_data, std::vector<std::shared_ptr<BlockOperation>> &ops)
      -> ChfsNullResult;

  virtual auto write_log_entry(off_t offset, const u8 *data, usize length) -> ChfsNullResult;
  ...
  virtual auto read_block_from_memory(block_id_t block_id, u8 *block_data, std::vector<std::shared_ptr<BlockOperation>> &ops)
      -> ChfsNullResult;
  ...
  auto get_log_start() -> u8* {
    return this -> block_data + (block_cnt - log_blk_num) * block_size();
  }

  auto get_log_end() -> u8* {
    return this -> block_data + block_cnt * block_size();
  }

  auto get_log_blk_num() -> usize {
    return this -> log_blk_num;
  }
...
public:
  ...
  auto flush_cur_block_to_memory(std::vector<std::shared_ptr<BlockOperation>> &ops) -> ChfsNullResult {
    auto target_block_id =
        this->start_block_id + this->cur_block_off / bm->block_sz;
    return this->bm->write_block_to_memory(target_block_id, this->buffer.data(), ops);
  }
