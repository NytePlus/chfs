...
public:
  ...
  auto allocate_with_log(std::vector<std::shared_ptr<BlockOperation>> &ops) -> ChfsResult<block_id_t>;
  ...
  auto deallocate_with_log(block_id_t block_id, std::vector<std::shared_ptr<BlockOperation>> &ops) -> ChfsNullResult;
};
