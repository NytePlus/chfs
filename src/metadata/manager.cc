...
auto InodeManager::allocate_inode(InodeType type, block_id_t bid)
  ...
  for (auto iter = iter_res.unwrap(); iter.has_next();
    ...
    if (free_idx) {
      ...
      Inode * inode_p = new Inode(type, bm -> block_size());
      inode_p -> flush_to_buffer(buffer.data());
...
auto InodeManager::allocate_inode_with_log(InodeType type, block_id_t bid, std::vector<std::shared_ptr<BlockOperation>> &ops)
    -> ChfsResult<inode_id_t> {
  auto iter_res = BlockIterator::create(this->bm.get(), 1 + n_table_blocks,
                                        1 + n_table_blocks + n_bitmap_blocks);
  if (iter_res.is_err()) {
  ...
  for (auto iter = iter_res.unwrap(); iter.has_next();
    ...
    for(usize i = 0; i < ops.size(); i ++)
      if(ops[i] -> block_id_ == count + 1 + n_table_blocks){
        in_txn = true;
        txn_id = i;
      }
    std::vector<u8> memory_data(bm -> block_size());
    if(in_txn){
      memcpy(memory_data.data(), ops[txn_id] -> new_block_state_.data(), bm -> block_size());
      bitmap = Bitmap(memory_data.data(), bm -> block_size());
    }
    auto free_idx = bitmap.find_first_free();
    ...
    if (free_idx) {
      ...
      if(in_txn){
        bm -> write_block_to_memory(count + 1 + n_table_blocks, memory_data.data(), ops);
      }else{
        auto res = iter.flush_cur_block_to_memory(ops);
        if (res.is_err()) {
          return ChfsResult<inode_id_t>(res.unwrap_error());
        }
      }
      ...
      Inode * inode_p = new Inode(type, bm -> block_size());
      inode_p -> flush_to_buffer(buffer.data());
      bm->write_block_to_memory(bid, buffer.data(), ops);
      set_table(free_idx.value(), bid);
      return ChfsResult<inode_id_t>(RAW_2_LOGIC(free_idx.value()));
    }
  }
...
auto InodeManager::set_table(inode_id_t idx, block_id_t bid) -> ChfsNullResult {
  ...
  // whose index is `idx`.
  u64 entry_per_block = bm->block_size() / sizeof(block_id_t) * sizeof(u8);
  std::vector<block_id_t> buffer(entry_per_block);
  block_id_t block_id = idx / entry_per_block + 1;
  block_id_t block_off = idx % entry_per_block;
  bm->read_block(block_id, reinterpret_cast<u8*>(buffer.data()));
  buffer[block_off] = bid;
  bm->write_block(block_id, reinterpret_cast<u8*>(buffer.data()));

  return KNullOk;
}

auto InodeManager::set_table_in_memory(inode_id_t idx, block_id_t bid, std::vector<std::shared_ptr<BlockOperation>> &ops) -> ChfsNullResult {
  ...
  block_id_t block_id = idx / entry_per_block + 1;
  block_id_t block_off = idx % entry_per_block;
  bm->read_block_from_memory(block_id, reinterpret_cast<u8*>(buffer.data()), ops);
  buffer[block_off] = bid;
  bm->write_block_to_memory(block_id, reinterpret_cast<u8*>(buffer.data()), ops);
...
auto InodeManager::get(inode_id_t id) -> ChfsResult<block_id_t> {
  ...
  // table index.
  if(id >= get_max_inode_supported()) 
    return ChfsResult<block_id_t>(KInvalidBlockID);
  u64 entry_per_block = bm->block_size() / sizeof(block_id_t) * sizeof(u8);
  block_id_t bid = 1 + LOGIC_2_RAW(id) / entry_per_block;
  std::vector<block_id_t> buffer(entry_per_block);
  bm->read_block(bid, reinterpret_cast<u8*>(buffer.data()));
  res_block_id = buffer[LOGIC_2_RAW(id) % entry_per_block];

  return ChfsResult<block_id_t>(res_block_id);
}

auto InodeManager::get_from_memory(inode_id_t id, std::vector<std::shared_ptr<BlockOperation>> &ops) -> ChfsResult<block_id_t> {
  block_id_t res_block_id = 0;
  ...
  block_id_t bid = 1 + LOGIC_2_RAW(id) / entry_per_block;
  std::vector<block_id_t> buffer(entry_per_block);
  bm->read_block_from_memory(bid, reinterpret_cast<u8*>(buffer.data()), ops);
  res_block_id = buffer[LOGIC_2_RAW(id) % entry_per_block];
...
auto InodeManager::get_type_from_memory(inode_id_t id, std::vector<std::shared_ptr<BlockOperation>> &ops) -> ChfsResult<InodeType> {
  std::vector<u8> buffer(bm->block_size());
  auto res = this->read_inode_from_memory(id, buffer, ops);
  if (res.is_err()) {
    return ChfsResult<InodeType>(res.unwrap_error());
  }
  Inode *inode_p = reinterpret_cast<Inode *>(buffer.data());
...
auto InodeManager::read_inode(inode_id_t id, std::vector<u8> &buffer)
  ...
  if (id >= max_inode_supported - 1) {
    if(id == 1) std::cout << "id >= max_inode_supported - 1 ErrorType::INVALID_ARG\n";
    return ChfsResult<block_id_t>(ErrorType::INVALID_ARG);
  }

  auto block_id = this->get(id);
  if (block_id.is_err()) {
    if(id == 1) std::cout << "block_id.unwrap_error()\n";
    return ChfsResult<block_id_t>(block_id.unwrap_error());
  }

  if (block_id.unwrap() == KInvalidBlockID) {
    if(id == 1) std::cout << "block_id.unwrap() == KInvalidBlockID ErrorType::INVALID_ARG\n" << "KInvalidBlockID=" << KInvalidBlockID << std::endl;
    return ChfsResult<block_id_t>(ErrorType::INVALID_ARG);
  }

  auto res = bm->read_block(block_id.unwrap(), buffer.data());
  if (res.is_err()) {
    if(id == 1) std::cout << "res.unwrap_error()\n";
    return ChfsResult<block_id_t>(res.unwrap_error());
  }
  return ChfsResult<block_id_t>(block_id.unwrap());
}

auto InodeManager::read_inode_from_memory(inode_id_t id, std::vector<u8> &buffer, std::vector<std::shared_ptr<BlockOperation>> &ops)
    -> ChfsResult<block_id_t> {
  if (id >= max_inode_supported - 1) {
    if(id == 1) std::cout << "id >= max_inode_supported - 1 ErrorType::INVALID_ARG\n";
    return ChfsResult<block_id_t>(ErrorType::INVALID_ARG);
...
auto InodeManager::free_inode_with_log(inode_id_t id, std::vector<std::shared_ptr<BlockOperation>> &ops) -> ChfsNullResult {
  ...
  set_table(LOGIC_2_RAW(id), 0);
  std::vector<u8> buffer(bm->block_size());
  block_id_t bitmap_block_id = 1 + n_table_blocks + LOGIC_2_RAW(id) / bm -> block_size() / 8; 
  bm->read_block_from_memory(bitmap_block_id, buffer.data(), ops);
  Bitmap(buffer.data(), bm->block_size()).clear(LOGIC_2_RAW(id) % (bm -> block_size() * 8));
  bm->write_block_to_memory(bitmap_block_id, buffer.data(), ops);
