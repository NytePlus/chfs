...
auto Inode::write_indirect_block_with_log(std::shared_ptr<BlockManager> &bm,
                                 std::vector<u8> &buffer, std::vector<std::shared_ptr<BlockOperation>> &ops) -> ChfsNullResult {
  if (this->blocks[this->nblocks - 1] == KInvalidBlockID) {
    return ChfsNullResult(ErrorType::INVALID_ARG);
  }

  return bm->write_block_to_memory(this->blocks[this->nblocks - 1], buffer.data(), ops);
}
