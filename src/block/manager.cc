...
auto BlockManager::write_log_entry(off_t offset, const u8 *data, usize length)
    -> ChfsNullResult {
  if(offset > log_blk_num * block_size())
    return ErrorType::INVALID;
  memcpy(block_data + offset + (block_cnt - log_blk_num) * block_size(), data, length);
...
auto BlockManager::write_block(block_id_t block_id, const u8 *data)
  ...
  // TODO: Implement this function.
  memcpy(block_data + block_id * block_size(), data, block_size());
  this->write_fail_cnt++;

  return KNullOk;
}

auto BlockManager::write_block_to_memory(block_id_t block_id, const u8 *data, std::vector<std::shared_ptr<BlockOperation>> &ops)
    -> ChfsNullResult {
  ...
  memcpy(buf.data(), data, DiskBlockSize);
  ops.push_back(std::make_shared<BlockOperation>(block_id, buf));
...
auto BlockManager::write_partial_block(block_id_t block_id, const u8 *data,
  ...
  memcpy(block_data + block_id * block_size() + offset, data, len);
  this->write_fail_cnt++;
  return KNullOk;
}
...
auto BlockManager::read_block(block_id_t block_id, u8 *data) -> ChfsNullResult {
  ...
  if(block_id >= block_cnt)
    return ChfsNullResult(ErrorType::INVALID_ARG);
  memcpy(data, block_data + block_id * block_size(), block_size());
  return KNullOk;
}

auto BlockManager::read_block_from_memory(block_id_t block_id, u8 *data, std::vector<std::shared_ptr<BlockOperation>> &ops)
    -> ChfsNullResult {
  if(block_id >= block_cnt){
    return ChfsNullResult(ErrorType::INVALID_ARG);
  }
  ...
  if(in_txn)
    memcpy(data, ops[txn_id] -> new_block_state_.data(), block_size());
  else
    memcpy(data, block_data + block_id * block_size(), block_size());
...
auto BlockManager::zero_block(block_id_t block_id) -> ChfsNullResult {
  ...
  // TODO: Implement this function.
  memset(block_data + block_id * block_size(), 0, block_size());
