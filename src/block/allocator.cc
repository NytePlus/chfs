...
auto BlockAllocator::allocate() -> ChfsResult<block_id_t> {
  ...
  for (uint i = 0; i < this->bitmap_block_cnt; i++) {
    ...
    if (res) {
      ...
      Bitmap(buffer.data(), bm->block_size()).set(*res);
      auto res1 = bm->write_block(this->bitmap_block_id + i, buffer.data());
      if(res1.is_err()){
        std::cout << "write block err\n";
        return ChfsResult<block_id_t>(ErrorType::INVALID);
      }
      retval = i * bm -> block_size() * 8 + *res;
...
auto BlockAllocator::allocate_with_log(std::vector<std::shared_ptr<BlockOperation>> &ops) -> ChfsResult<block_id_t> {
  std::vector<u8> buffer(bm->block_size());

  for (uint i = 0; i < this->bitmap_block_cnt; i++) {
    bm->read_block_from_memory(i + this->bitmap_block_id, buffer.data(), ops);
    ...
    if (res) {
      ...
      // 3. Calculate the value of `retval`.
      Bitmap(buffer.data(), bm->block_size()).set(*res);
      auto res1 = bm->write_block_to_memory(this->bitmap_block_id + i, buffer.data(), ops);
      if(res1.is_err()){
        std::cout << "write block err\n";
        return ChfsResult<block_id_t>(ErrorType::INVALID);
      }
...
auto BlockAllocator::deallocate(block_id_t block_id) -> ChfsNullResult {
  ...
  std::vector<u8> buffer(bm->block_size());
  block_id_t id = block_id / (this -> bm -> block_size() * 8) + bitmap_block_id;
  block_id_t bit = block_id % (this -> bm -> block_size() * 8);

  this -> bm->read_block(id, buffer.data());
  if(Bitmap(buffer.data(), bm->block_size()).check(bit))
    Bitmap(buffer.data(), bm->block_size()).clear(bit);
  else return ChfsNullResult(ErrorType::INVALID_ARG);
...
auto BlockAllocator::deallocate_with_log(block_id_t block_id, std::vector<std::shared_ptr<BlockOperation>> &ops) -> ChfsNullResult {
  if (block_id >= this->bm->total_blocks()) {
    return ChfsNullResult(ErrorType::INVALID_ARG);
  }
  // TODO: Implement this function.
  ...
  auto res = bm->write_block_to_memory(id, buffer.data(), ops);
  if(res.is_err())
    return ChfsNullResult(ErrorType::OUT_OF_RESOURCE);
  return KNullOk;
}
