...
auto FileOperation::remove_file_with_log(inode_id_t id, std::vector<std::shared_ptr<BlockOperation>> &ops) -> ChfsNullResult {
  ...
  {
    auto res = this->inode_manager_->free_inode_with_log(id, ops);
    if (res.is_err()) {
  ...
  for (auto bid : free_set) {
    auto res = this->block_allocator_->deallocate_with_log(bid, ops);
    if (res.is_err()) {
      return res;
    }
  }
